...
说明
=============
> * 服务端要求：Ubuntu14.04或者CentOS-6.*或者Debian 8
> * 客户端：
 - iOS=>ikev2
 - Windows10平台=>ikev2
> * 可使用自己的私钥和根证书，也可自动生成
...
1.下载脚本:
```shell
 wget https://github.com/war10ck-efz/one-key-ikev2-vpn.git
```
...
4.输入两次pkcs12证书的密码(不可为空，否则ios上无法使用)

5.看到install success字样即表示安装成功。默认用户名密码将以黄字显示，可根据提示自行修改文件中的用户名密码。

6.将提示信息中的证书文件ca.cert.pem拷贝到客户端，修改后缀名为.cer后导入。ios设备使用Ikev2另需导入client.cert.p12.
